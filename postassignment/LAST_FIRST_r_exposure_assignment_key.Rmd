...
> Notes from the instructor appear as block quotes like these. Leave these instructions in. Replace text outside block quotes and inside code chunks (like the one below) to answer questions.
...
```{r, echo=FALSE}
ggplot(germany, aes(x=year, y=gdpPercap)) + geom_line()
```
...
```{r}
ggplot(gapminder, aes(x=year, y=gdpPercap, group=country, color=continent)) + geom_line()
```
...
```{r}
continents <- gapminder %>% group_by(year, continent) %>% summarize(gdpPercap = mean(gdpPercap))
ggplot(continents, aes(x=year, y=gdpPercap, color = continent)) + geom_line() 
```
