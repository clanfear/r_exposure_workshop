...
date: "Sep 13, 2020<br>Updated: `r gsub(' 0', ' ', format(Sys.Date(), format='%b %d, %Y'))`"
output:
...
Maybe you need to calculate the mean and standard deviation of a bunch of columns. With **`across()`**, put the variables to compute over (using `c()` or `select()` syntax) and put the functions to use in a `list()` after.
...
`names_transform` tells `pivot_longer()` we want to convert `week` into an integer number.
